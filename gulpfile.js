var gulp = require("gulp"),
  ...
  src = {
    ...
    pluginsCssRelFolderRoot: "./src/assets/css/root-plugins-related/**/*",
...
function pluginsRelatedCssRoot(done) {
  gulp
    .src(src.pluginsCssRelFolderRoot)
    .pipe(
      customPlumber(
...
// gulp.task('allTask', gulp.series(fonts, php, html, media, cssCopy, styleCss, scss, jsCopy, 'images', 'svgGif'));
...
function watchFiles() {
  ...
  gulp.watch(
    src.pluginsCssRelFolderRoot,
    gulp.series(pluginsRelatedCssRoot, reload)
  );
  ...
  gulp.watch(src.images, gulp.series("images", "svgGif", reload));
}
